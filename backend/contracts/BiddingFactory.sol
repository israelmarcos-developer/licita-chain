...
contract BiddingFactory is AccessControl {
    ...
    modifier OnlyPublicOrg() {
        require(
            IERC721(GovToken).balanceOf(msg.sender) > 0,
            "You cannot use this functions"
        );
        _;
    }

    event NewBiddingEmitted(
        address indexed biddingAddress,
        uint256 indexed biddingValue,
